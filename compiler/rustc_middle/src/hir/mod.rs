...
impl ModuleItems {
    ...
    pub fn free_items(&self) -> impl use<'_> Iterator<Item = ItemId> {
        self.free_items.iter().copied()
    }

    pub fn trait_items(&self) -> impl use<'_> Iterator<Item = TraitItemId> {
        self.trait_items.iter().copied()
    }

    /// Returns all items that are associated with some `impl` block (both inherent and trait impl
    /// blocks).
    pub fn impl_items(&self) -> impl use<'_> Iterator<Item = ImplItemId> {
        self.impl_items.iter().copied()
    }

    pub fn foreign_items(&self) -> impl use<'_> Iterator<Item = ForeignItemId> {
        self.foreign_items.iter().copied()
    }

    pub fn owners(&self) -> impl use<'_> Iterator<Item = OwnerId> {
        self.free_items
            .iter()
            .map(|id| id.owner_id)
    ...
    pub fn definitions(&self) -> impl use<'_> Iterator<Item = LocalDefId> {
