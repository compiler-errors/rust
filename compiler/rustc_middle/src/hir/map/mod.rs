...
impl<'hir> Map<'hir> {
    ...
    #[inline]
    pub fn items(self) -> impl Iterator<Item = ItemId> + use<'hir> {
        self.tcx.hir_crate_items(()).free_items.iter().copied()
    }

    #[inline]
    pub fn module_items(self, module: LocalModDefId) -> impl Iterator<Item = ItemId> + use<'hir> {
    ...
    pub fn body_param_names(self, id: BodyId) -> impl Iterator<Item = Ident> + use<'hir> {
    ...
    #[inline]
    pub fn body_owners(self) -> impl Iterator<Item = LocalDefId> + use<'hir> {
    ...
    #[inline]
    pub fn parent_id_iter(self, current_id: HirId) -> impl Iterator<Item = HirId> + use<'hir> {
