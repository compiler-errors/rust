...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
    ...
    fn maybe_suggest_add_generic_impl_trait(
        ...
        match tcx.hir_node_by_def_id(parent_id) {
            hir::Node::Item(hir::Item {
                kind: hir::ItemKind::Struct(_, generics) | hir::ItemKind::Enum(_, generics),
                ..
            }) => {
                let Ok(rendered_ty) = self.tcx().sess.source_map().span_to_snippet(self_ty.span)
                else {
                    return false;
                };
                let param = "TUV"
