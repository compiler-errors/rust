...
fn appearances_iter(
    ...
    appearances: &Appearances,
) -> impl Iterator<Item = AppearanceIndex> + use<'_> {
...
impl LocalUseMap {
    ...
    pub(crate) fn defs(&self, local: Local) -> impl Iterator<Item = PointIndex> + use<'_> {
        appearances_iter(self.first_def_at[local], &self.appearances)
            .map(move |aa| self.appearances[aa].point_index)
    }

    pub(crate) fn uses(&self, local: Local) -> impl Iterator<Item = PointIndex> + use<'_> {
        appearances_iter(self.first_use_at[local], &self.appearances)
            .map(move |aa| self.appearances[aa].point_index)
    }

    pub(crate) fn drops(&self, local: Local) -> impl Iterator<Item = PointIndex> + use<'_> {
