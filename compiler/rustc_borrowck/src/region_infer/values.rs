...
impl LivenessValues {
    ...
    pub(crate) fn regions(&self) -> impl use<'_> Iterator<Item = RegionVid> {
    ...
    pub(crate) fn live_regions_unordered(&self) -> impl use<'_> Iterator<Item = RegionVid> {
    ...
    fn live_points(&self, region: RegionVid) -> impl use<'_> Iterator<Item = PointIndex> {
...
impl<N: Idx> RegionValues<N> {
    ...
    pub(crate) fn locations_outlived_by<'a>(&'a self, r: N) -> impl use<'a, N> Iterator<Item = Location> {
    ...
    pub(crate) fn universal_regions_outlived_by<'a>(
        &'a self,
        r: N,
    ) -> impl use<'a, N> Iterator<Item = RegionVid> {
    ...
    pub(crate) fn placeholders_contained_in<'a>(
        &'a self,
        r: N,
    ) -> impl use<'a, N> Iterator<Item = ty::PlaceholderRegion> {
        self.placeholders
            .row(r)
            .into_iter()
            .flat_map(|set| set.iter())
    ...
    pub(crate) fn elements_contained_in<'a>(
        &'a self,
        r: N,
    ) -> impl use<'a, N> Iterator<Item = RegionElement> {
