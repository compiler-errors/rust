...
pub fn relate_args_invariantly<I: Interner, R: TypeRelation<I>>(
    ...
    relation.tcx().mk_args_from_iter(iter::zip(a_arg.into_iter(), b_arg.into_iter()).map(
...
pub fn relate_args_with_variances<I: Interner, R: TypeRelation<I>>(
    ...
    let params = iter::zip(a_arg.into_iter(), b_arg.into_iter()).enumerate().map(|(i, (a, b))| {
        ...
        (ty::Tuple(as_), ty::Tuple(bs)) => {
            if as_.len() == bs.len() {
                Ok(Ty::new_tup_from_iter(
                    tcx,
                    iter::zip(as_.into_iter(), bs.into_iter()).map(|(a, b)| relation.relate(a, b)),
                )?)
