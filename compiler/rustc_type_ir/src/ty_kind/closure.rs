...
impl<I: Interner> ClosureArgs<I> {
    ...
    pub fn new(tcx: I, parts: ClosureArgsParts<I>) -> ClosureArgs<I> {
        ClosureArgs {
            args: tcx.mk_args_from_iter(parts.parent_args.into_iter().chain([
        ...
        match kind {
            ty::ClosureKind::Fn | ty::ClosureKind::FnMut => {
                ...
                Ty::new_tup_from_iter(
                    ...
                    tupled_inputs_ty
                        .tuple_fields()
                        .into_iter()
                        .chain(coroutine_captures_by_ref_ty.tuple_fields().into_iter()),
                )
            }
            ty::ClosureKind::FnOnce => Ty::new_tup_from_iter(
                ...
                tupled_inputs_ty
                    .tuple_fields()
                    .into_iter()
                    .chain(closure_tupled_upvars_ty.tuple_fields().into_iter()),
            ),
        }
    }
}
